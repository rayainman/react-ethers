...
const ERC721Assignment = () => {
  ...
  const getAccountBalance = async () => {
    const _totalSupply = await contract.balanceOf(signer.getAddress());
    // let balance = await provider.getBalance("ethers.eth");
    setAccountBalance(_totalSupply.toNumber());
  };
  ...
  const onMint = async () => {
    ...
    if (!contract || !mintPrice) {
      // currentAccount format is error
      // await contract.setIncrement({ from: currentAccount.address }); // ps. 補充 from 參數 https://docs.ethers.io/v5/api/contract/contract/
      return
    }
    ...
    let txn = await contract.mint(options);
